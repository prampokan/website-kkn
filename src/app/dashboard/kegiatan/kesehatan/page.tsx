...
export default function DashboardKesehatan() {
    ...
    const addKesehatan = async (e: any) => {
        e.preventDefault()
        if (!image) return;
        setUploading(true);
        try {
            const imageUrl = await uploadImage(image);
            await addDoc(collection(db, 'kegiatanKesehatan'), {
                judul: judul,
                deskripsi: deskripsi,
                image: imageUrl,
            });
    ...
    const deleteKesehatan = async (id: string, imageUrl: string) => {
        try {
          await deleteDoc(doc(db, 'kegiatanKesehatan', id));
                        ...
                        (kesehatan && kesehatan.map((item: any) => (
                            <>
                                <CardDashboard 
                                    ...
                                    image={item.image}
                                    onDelete={() => deleteKesehatan(item.id, item.image)}
                                />
                            </>
                        )))
                    }
